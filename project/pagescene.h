...
public:
    ...
    PageScene(const BookEmbeddedGraph* g, const int p, MainWindow *w, QColor col, QLabel* pageNumber, QLabel* crossings, QPushButton* del, int width, int height);

    /**
     * @brief PageScene::setSize We adjust the positions of the nodes in the scene. At the ends, we leave a small margin and some space for node moving. Then, we divide the rest of the new width equally between the nodes. Then, we calculate how high the edges can go. Attention: the scene is not redrawn automatically.
     */
